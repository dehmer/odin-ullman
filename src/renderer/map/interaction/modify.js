 ...
 */
class Modify extends olInteraction.Modify {
...
export default features => {
  ...
  features.on('add', ({ element }) => {
    if (features.getLength() <= 1) source.addFeature(element)
    else source.clear()
  })

  features.on('remove', ({ element }) => {
    if (!source.hasFeature(element)) return
